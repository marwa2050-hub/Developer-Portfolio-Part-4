...
const ContactForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(v).length === 0) {
      // push feedback and save
      const newFb = { name: data.name || "Anonymous", rating: Math.floor(Math.random() * 5) + 1, comment: data.message || "—" , date: new Date().toISOString() };
  ...
  return (
    <section className="contact container">
      ...
      <form className="contact-form" onSubmit={handleSubmit} noValidate>
        <label>
          <input name="name" placeholder="Your name" value={data.name} onChange={handleChange} />
          {errors.name && <small className="error">{errors.name}</small>}
        </label>

        <label>
          <input name="email" placeholder="Email (optional)" value={data.email} onChange={handleChange} />
          {errors.email && <small className="error">{errors.email}</small>}
        </label>

        <label>
          <textarea name="message" placeholder="Message" value={data.message} onChange={handleChange} rows="6" />
      ...
      <div className="feedback-wall">
        ...
        {feedbacks.map((fb, i) => (
          <FeedbackCard key={i} name={fb.name} rating={fb.rating} comment={fb.comment || fb.comment} />
        ))}
      </div>
    </section>
  );
